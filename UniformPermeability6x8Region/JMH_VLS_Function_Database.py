...
# Description: This code is a database of supporting functions for the JMH_VQE_Pitchfork_Fracture_Execution.py
# program. That script operates by sequentially running functions contained within this file. For more detailed
# documentation, see the beginning of each function.

# Changelog:
# 7/7/21: Initiate code 'usability' overhaul to make sure everything is user-friendly going forward.  Begin changelog.
# 7/13/21: Add code to load in results that should be used as a starting point and to store those in variables that 
# will not be overwritten when loading results from a subsequent training process.
# 7/13/21: Add code to the train_with_shot noise process that can use a previous set of parameters to train a modified
# pitchfork problem. Also add the ability to modify this initial set of parameters using a random vector with a specified
# norm.
# 7/13/21: Modify the calculate_fidelity() function so that if the first array passed in is not nested, an error will not 
# be thrown, and the fidelity can be calculated accordingly.  (Simply add a try-except, so that both situations of nested and 
# unnested x-arrays work.)
# 7/14/21: Update train_with_no_noise to use loaded parameters and/or to have varied versions of those initial parameters
# when solving from a previous solution.
# 7/16/21: Add code to run the SPSA optimization from Qiskit.
# 7/19/21: Comment out code for computing and plotting trace distance, as that 
# has become a less important part of our investigation.
# 7/22/21: Add code for computing a non-black-box-optimized gradient descent.
# 8/2/21: Save all plots as pdfs, and not pngs, by default. Also, modify default figure size to be (10, 5) instead of (15, 5).
# 8/4/21: Update name of file to be consistent with new name for algorithm. (VLS, not VQLS)
